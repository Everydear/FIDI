...
export function analyzeSeries(points, asOf) {
  ...
  const rsi14 = calculateRsi(
    prices.map((point) => point.value),
    14,
  );
  const movingAverage20 = mean(
    prices.slice(-20).map((point) => point.value),
  );
  const trendStrength20 = latest.value / movingAverage20 - 1;
  const momentum60Score = squash(return60, 0.25);
  const momentum20Score = squash(return20, 0.12);
  const riskAdjustedMomentumScore = squash(
    return60 / Math.max(volatility20, 0.05),
    1.5,
  );
  const trendScore = squash(trendStrength20, 0.08);
  const rsiScore = squash((rsi14 - 50) / 25, 1);
  const drawdownPenalty = squash(Math.max(0, -drawdown60), 0.2);
  const volatilityPenalty = squash(Math.max(0, volatility20 - 0.2), 0.2);
  const score =
    0.28 * momentum60Score +
    0.18 * momentum20Score +
    0.18 * riskAdjustedMomentumScore +
    0.18 * trendScore +
    0.1 * rsiScore -
    0.04 * drawdownPenalty -
    0.04 * volatilityPenalty;
  ...
  return {
    ...
    drawdown60,
    rsi14,
    rsiStatus: rsiStatus(rsi14),
    aboveMovingAverage20: latest.value >= movingAverage20,
    trendStrength20,
    scoreBreakdown: {
      momentum60: momentum60Score,
      momentum20: momentum20Score,
      riskAdjustedMomentum: riskAdjustedMomentumScore,
      trend: trendScore,
      rsi: rsiScore,
      drawdownPenalty,
      volatilityPenalty,
...
export function buildDailyGuide({
  ...
  return {
    version: "KRW-V4.3-SIGNAL-20260801",
    scoreMethod:
      "신호점수 v3 · 60·20일 모멘텀 + 위험조정 수익률 + 추세·RSI(14) - 낙폭·변동성",
    asOf,
    profile,
