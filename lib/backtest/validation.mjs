...
function historicalQuantile(sortedValues, probability) {
...
function drawdownDuration(curve) {
  ...
  for (const point of curve) {
    if (point.drawdown < -1e-12) {
      if (currentObservations === 0) currentStart = point.date;
      currentObservations += 1;
      if (currentObservations > maximumObservations) {
        maximumObservations = currentObservations;
        maximumStart = currentStart;
        maximumEnd = point.date;
      }
    } else {
  ...
  const durationDays =
    maximumStart && maximumEnd
      ? Math.max(
          0,
          Math.round(
            (Date.parse(`${maximumEnd}T00:00:00Z`) -
              Date.parse(`${maximumStart}T00:00:00Z`)) /
              86_400_000,
          ),
        )
      : 0;
...
export function calculateRiskAnalytics(curve) {
  ...
  if (returns.length < 2) {
    return {
      ...
      var95DailyLoss: null,
      cvar95DailyLoss: null,
      var99DailyLoss: null,
      cvar99DailyLoss: null,
      downsideDeviation: null,
      sortino: null,
      maximumDrawdown: null,
      maximumDrawdownDurationObservations: 0,
      maximumDrawdownDurationDays: 0,
      maximumDrawdownStart: null,
      maximumDrawdownEnd: null,
      riskLevel: "LIMITED_DATA",
    };
  }

  const sorted = [...returns].sort((left, right) => left - right);
  const quantile95 = historicalQuantile(sorted, 0.05);
  const quantile99 = historicalQuantile(sorted, 0.01);
  const tail95 = returns.filter((value) => value <= quantile95);
  const tail99 = returns.filter((value) => value <= quantile99);
  ...
  const elapsedDays = Math.max(
    1,
    (Date.parse(`${points.at(-1).date}T00:00:00Z`) -
      Date.parse(`${points[0].date}T00:00:00Z`)) /
      86_400_000,
  );
  const cagr =
    (points.at(-1).value / points[0].value) ** (365.25 / elapsedDays) - 1;
  const maximumDrawdown = Math.min(
    ...points.map((point) => point.drawdown ?? 0),
  );
  const duration = drawdownDuration(points);
  const var95DailyLoss = Math.max(0, -quantile95);
  const var99DailyLoss = Math.max(0, -quantile99);
  const cvar95DailyLoss = Math.max(0, -mean(tail95));
  const cvar99DailyLoss = Math.max(0, -mean(tail99));
  const riskLevel =
    cvar95DailyLoss >= 0.04 || maximumDrawdown <= -0.4
      ? "HIGH"
      : cvar95DailyLoss >= 0.025 || maximumDrawdown <= -0.25
        ? "MEDIUM"
        : "LOW";
  ...
  return {
    ...
    cvar99DailyLoss,
    downsideDeviation,
    sortino: downsideDeviation > 0 ? cagr / downsideDeviation : null,
    maximumDrawdown,
    maximumDrawdownDurationObservations: duration.observations,
    maximumDrawdownDurationDays: duration.days,
    maximumDrawdownStart: duration.start,
    maximumDrawdownEnd: duration.end,
    riskLevel,
  };
}
...
function classifyRegime(benchmarkCagr) {
  if (benchmarkCagr >= 0.1) return "UP";
  if (benchmarkCagr <= -0.1) return "DOWN";
  return "SIDEWAYS";
}
...
function summarizeRegimes(folds) {
  ...
  return ["UP", "DOWN", "SIDEWAYS"].flatMap((regime) => {
    const matches = folds.filter((fold) => fold.regime === regime);
    if (!matches.length) return [];
    const excess = matches.map((fold) => fold.excessCagr);
    return [{
      regime,
      label: labels[regime],
      folds: matches.length,
      beatRate: excess.filter((value) => value > 0).length / excess.length,
      meanExcessCagr: mean(excess),
      worstExcessCagr: Math.min(...excess),
    }];
  });
}
...
export function calculateWalkForwardStatistics(
  ...
  if (points.length < minimumTrainObservations + testWindow) {
    return {
      ...
      regimeBreakdown: [],
      worstFold: null,
      validationMethod: "expanding-window-temporal-holdout-v2",
    };
  }
  ...
  for (
    ...
    folds.push({
      ...
      maximumDrawdown,
      benchmarkCagr,
      regime: classifyRegime(benchmarkCagr),
    });
  ...
  const meanExcessCagr = excess.length
    ...
    : null;
  const confidence = confidenceInterval95(excess);
  const worstFold = folds.length
    ? folds.reduce((worst, fold) =>
        fold.excessCagr < worst.excessCagr ? fold : worst,
      )
    : null;
  return {
    status:
    ...
    regimeBreakdown: summarizeRegimes(folds),
    worstFold,
    validationMethod: "expanding-window-temporal-holdout-v2",
  };
}
    ...
    },
    {
      ...
      detail:
        walkForwardEvidence.meanExcessCagr === null
          ...
          : `${walkForwardEvidence.folds.length}개 검증 구간 · 상회 비율 ${(walkForwardEvidence.beatRate * 100).toFixed(1)}% · 평균 연수익률 차이 ${(walkForwardEvidence.meanExcessCagr * 100).toFixed(2)}%p · 최악 ${(walkForwardEvidence.worstExcessCagr * 100).toFixed(2)}%p`,
      guide: "확장 기준 구간과 다음 검증 구간을 순서대로 반복했습니다. 95% 범위와 상승·하락·횡보 구간을 함께 확인하며 미래 수익을 보장하지는 않습니다.",
    },
    {
      id: "risk-limits",
